...
class Storage(object):
    ...
    def get_image_bucket(self, bucket, image_path):
        try:
            ...
            image_bytes = (
                self.s3.get_object(Bucket=bucket, Key=image_path).get("Body").read()
            )
    ...
    def get_release_bucket(self, bucket, release_path):
        try:
            # Verifique se o bucket existe
            existing_buckets = [b["Name"] for b in self.s3.list_buckets()["Buckets"]]
            if bucket not in existing_buckets:
            ...
            # Obtenha o release do S3
            release_bytes = (
                self.s3.get_object(Bucket=bucket, Key=release_path).get("Body").read()
            )

            # Retorne o release
            return release_bytes, "Sucesso"
        except Exception as e:
            return None, str(e)
